...
VulkanApplication::VulkanApplication(const char * title, uint32_t width, uint32_t height) {
    glfwInit();
...
Window * createWindow(const char * title, uint32_t width, uint32_t height) {
    ...
    try {
        window->app = std::make_unique<VulkanApplication>(title, width, height);
